...
const checkFreeScheduleUser = async (body) => {
  ...
  try {
    const sdate = moment(start_date).startOf("day").toDate();
    const edate = moment(end_date).endOf("day").toDate();

    //Lấy ra booking bị trùng trong khoản startDate và endDate
    const availableEmployees = await User.find({
      role: "employee",
      _id: {
        $nin: await bookingModel
          .find({
            ...
            $or: [
              ...
              {
                start_date: { $gte: sdate, $lte: edate },
                end_date: { $gte: sdate, $lte: edate },
              },
            ],
          })
